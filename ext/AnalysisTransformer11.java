   ...
   //     }
    ...
    private State transfer(Unit u, State in) {
        ...
        // Store
            ...
            for (String obj : baseObjs) {
                ...
                String killKey = obj + "." + field;

                // remove all entries matching this location
                out.available.removeIf(e -> e.endsWith("|" + killKey));
            }
        }
            ...
            for (String obj : in.stack.get(base)) {
                ...
                if (in.heap.containsKey(obj)
                    ...
                    if (left instanceof Local) {
                        ...
                        String entry = ((Local) left).getName()
                                + "|" + key;

                        out.available.add(entry);
                    }
                }
            }
    ...
    private void checkRedundantLoads(Body body,
        ...
        for (Unit u : body.getUnits()) {
            ...
            String replacementLocal = null;

            for (String entry : in.available) {

                
                String[] firstSplit = entry.split("\\|", 2);
                String local = firstSplit[0];
                String rest = firstSplit[1];

                String[] secondSplit = rest.split("\\.", 2);
                String obj = secondSplit[0];
                String fld = secondSplit[1];

                if (in.heap.containsKey(obj)
                        && in.heap.get(obj).containsKey(fld)) {
                        
                    Set<String> val =
                            in.heap.get(obj).get(fld);
                        
                    if (val.equals(loadedObjs)) {
                    
                        valueAvailable = true;
                        replacementLocal = local;
                        break;
                    }
                }
            }
